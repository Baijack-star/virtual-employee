...
async def execute_openai_research(topic: str, run_id: str = None) -> dict:
    ...
    if not AGENTS_AVAILABLE: # Check if the core 'agents' module was imported successfully
    ...
    try:
        ...
        if not report_obj: # Should not happen if run_research always returns a ResearchReport
        ...
        if report_obj.collected_facts and not report_obj.error_message :
            ...
            for fact in report_obj.collected_facts[:3]: # Show first 3 facts as part of details
        ...
        if not response_details and report_obj.report and not report_obj.error_message:
            ...
            response_details.append("No specific details to summarize here, check full report or error message.")
    ...
    except ImportError as e: # Catch potential ImportErrors if AGENTS_AVAILABLE was True but something specific failed later
        ...
        return { "session_id": run_id, "summary": "Error: Critical Component Missing", "details": [error_msg], "status": "error_configuration", "title": "Configuration Error", "outline": [], "report": error_msg, "sources": [], "word_count": 0, "collected_facts": [], "error_message": error_msg }
