    ...
    agents = [
        {
            "id": "writer",
            "name": "文案策划员",
            "description": "擅长撰写各类营销文案、广告语、社交媒体内容等。",
            "type": "standard",
        },
        {
            "id": "data_analyst",
            "name": "数据分析员",
            "description": "能够处理和分析数据，提取洞见，并生成报告。",
            "type": "standard",
        },
        {
            "id": "video_producer",
            "name": "视频制作员",
            "description": "可以协助完成视频脚本编写、剪辑和后期制作。",
            "type": "standard",
        },
        {
            "id": "research_assistant",
            "name": "深度研究助理",
            "description": "输入研究主题或问题，获取由AI驱动的研究报告。",
            "type": "research",
        },
    ]
...
async def event_generator(session_id: str, agent_type: str):
    logger.info(
        f"SSE connection established for session_id: {session_id}, agent_type: {agent_type}"
    )
    ...
    logger.info(
        f"SSE stream placeholder work done for session_id: {session_id}, agent_type: {agent_type}"
    )
...
async def agent_status_sse(request: Request, agent_type: str, session_id: str):
    return StreamingResponse(
        event_generator(session_id, agent_type), media_type="text/event-stream"
    )


@app.post(
    "/api/agent/research_assistant/invoke", response_model=ResearchAssistantResponse
)
async def invoke_research_assistant(request_data: ResearchTaskRequest):
    run_id = str(uuid.uuid4())
    logger.info(
        f"[{run_id}] Received research task via API: '{request_data.task_description}'"
    )

    research_result_dict = await execute_openai_research(
        topic=request_data.task_description, run_id=run_id
    )
    ...
    try:
        ...
        logger.error(
            f"[{run_id}] Error creating ResearchAssistantResponse Pydantic model from research_result_dict: {e}",
            exc_info=True,
        )
        ...
        return ResearchAssistantResponse(
            ...
            ],  # Make sure details is a list of strings
    ...
    logger.info(
        f"[{run_id}] Successfully processed research task. Status: {response_obj.status}"
    )
    return response_obj
