...
from dotenv import load_dotenv # Not called directly, but available
import logging
...
try:
    ...
    logger.error(f"Critical: 'agents' module not found. Error: {e}. Please ensure 'openai-agents' is installed for full functionality.")
    # This will halt execution if the module is critical at import time.
    raise ImportError("Core 'agents' module (openai-agents) not found. Please install the required dependencies.") from e
...
@function_tool
async def save_important_fact(fact: str, source: str = None, context: dict = None) -> str:
    if context is None or "collected_facts_list" not in context:
        logger.warning("'context' with 'collected_facts_list' not provided to save_important_fact. Fact not saved.")
        return "Fact received, but no context or list provided to save it."

    collected_facts_list = context["collected_facts_list"]
    fact_data = {"fact": fact, "source": source or "Not specified", "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
    ...
    if isinstance(collected_facts_list, list):
        ...
        logger.error(f"Failed to save fact: 'collected_facts_list' in context is not a list. Type: {type(collected_facts_list)}")
...
) if AGENTS_AVAILABLE else None # Define as None if SDK not available
...
async def run_research(topic: str, trace_group_id: str = None) -> ResearchReport:
    ...
    if not AGENTS_AVAILABLE or not research_agent or not editor_agent or not triage_agent:
        logger.error(f"[{run_id}] Core agents are not available (AGENTS_AVAILABLE={AGENTS_AVAILABLE}). Cannot run research.")
        return ResearchReport(title=f"Agent Initialization Error for topic '{topic}'", report="Core research agents could not be initialized. Check 'openai-agents' installation.", error_message="Core agents not available.", outline=[], sources=[], word_count=0, collected_facts=[])

    if not os.getenv("OPENAI_API_KEY"):
        logger.error(f"[{run_id}] OPENAI_API_KEY environment variable is not set.")
        return ResearchReport(title=f"Configuration Error for topic '{topic}'", report="OPENAI_API_KEY is not set.", error_message="OPENAI_API_KEY is not set.", outline=[], sources=[], word_count=0, collected_facts=[])
    ...
    if 'trace' in globals(): # Check if trace was successfully imported
        try:
            ...
            logger.warning(f"[{run_id}] Failed to initialize 'trace' context manager: {e}. Proceeding without it.")
    ...
    async def research_workflow():
        ...
        try:
            ...
            if not hasattr(triage_result, 'final_output') or not isinstance(triage_result.final_output, ResearchPlan):
                logger.error(f"[{run_id}] Triage agent did not return a valid ResearchPlan. Output: {triage_result.final_output}")
                final_report_obj = ResearchReport(title=f"Triage Error for {topic}", outline=[], report="Triage agent output was not a valid ResearchPlan.", sources=[], word_count=0, error_message="Triage agent output was not a valid ResearchPlan.", collected_facts=current_run_collected_facts)
                return

            research_plan = triage_result.final_output
            logger.info(f"[{run_id}] Research Plan generated: {research_plan.dict(exclude_none=True)}")
            ...
            if triage_result and hasattr(triage_result, 'history'):
                ...
                for item in reversed(triage_result.history):
                    if hasattr(item, 'agent_name') and item.agent_name == "Editor Agent" and \
                       hasattr(item, 'output') and isinstance(item.output, ResearchReport):
                        final_report_obj = item.output
                        editor_output_found = True
                        logger.info(f"[{run_id}] Editor Agent output successfully retrieved from history.")
                        break
                if not editor_output_found and isinstance(triage_result.final_output, ResearchReport):
                    final_report_obj = triage_result.final_output
                    logger.info(f"[{run_id}] Editor Agent output retrieved as final output of the chain.")
                elif not editor_output_found:
                    logger.warning(f"[{run_id}] Could not find ResearchReport from Editor Agent in history or final output.")
                    final_report_obj = ResearchReport(title=f"Report Generation Incomplete for {topic}", outline=[], report="Editor agent did not produce a final report in the expected format.", sources=[], word_count=0, error_message="Editor report not found.", collected_facts=current_run_collected_facts)
            else:
                 logger.warning(f"[{run_id}] Triage result has no history or unexpected structure.")
                 final_report_obj = ResearchReport(title=f"Processing Error for {topic}", outline=[], report="Could not determine final report due to processing error.", sources=[], word_count=0, error_message="Triage result processing error.", collected_facts=current_run_collected_facts)

            logger.info(f"[{run_id}] Research workflow complete for topic: '{topic}'.")

        except Exception as e:
            logger.error(f"[{run_id}] Error during research workflow execution: {e}", exc_info=True)
