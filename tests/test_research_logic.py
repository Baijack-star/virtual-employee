...
def mock_successful_runner_run_result():
    ...
    """
    triage_output = ResearchPlan(topic="Test Topic", search_queries=["q1"], focus_areas=["fa1"])
    ...
    editor_output = ResearchReport(
        ...
        collected_facts=[{"fact": "A key fact discovered", "source": "Test Source", "timestamp": "2023-01-01 10:00:00"}]
    )

    class MockAgentRunResult:
        def __init__(self, final_output, history=None):
            self.final_output = final_output # This would be editor_output for the whole chain
            self.history = history if history else []
            self.to_input_list = mock.Mock(return_value=[{"role": "user", "content": "Mocked input for next agent"}])

    # Simulate history: Triage -> Research -> Editor
    history_log = [
        mock.Mock(agent_name="Triage Agent", output=triage_output),
        mock.Mock(agent_name="Research Agent", output="Intermediate research notes..."), # Research agent's output
        mock.Mock(agent_name="Editor Agent", output=editor_output) # Editor agent's final output
    ]
...
@mock.patch("app.core_research_logic.agent_based_research.AGENTS_AVAILABLE", True) # Ensure SDK is "available"
@mock.patch("app.core_research_logic.agent_based_research.Runner.run")
async def test_run_research_success(mock_runner_run_method, mock_successful_runner_run_result_fixture):
    ...
    report = await run_research(topic="test successful topic", trace_group_id="trace-success-run")
    ...
    assert report.collected_facts == [{"fact": "A key fact discovered", "source": "Test Source", "timestamp": "2023-01-01 10:00:00"}]
...
async def test_run_research_no_api_key():
    with mock.patch.dict(os.environ, {}, clear=True):
        report = await run_research(topic="test topic no key", trace_group_id="trace-no-key")
    assert report is not None
...
async def test_run_research_runner_exception(mock_runner_run_method):
    ...
    report = await run_research(topic="test topic exception", trace_group_id="trace-runner-exc")

    assert report is not None
    assert "Simulated LLM API Error" in report.error_message
    assert "Workflow Error" in report.error_message # As generated by run_research's exception handling
...
async def test_save_important_fact_tool_no_context_or_list():
    ...
    result_msg_no_ctx = await save_important_fact("fact_no_ctx", "source_no_ctx", context=None)
    assert "no context or list provided" in result_msg_no_ctx

    # Test when context is an empty dict (missing 'collected_facts_list')
    empty_context = {}
    result_msg_empty_dict = await save_important_fact("fact_empty_dict", "source_empty_dict", context=empty_context)
    ...
    result_msg_invalid_list = await save_important_fact("fact_invalid_list", "source_invalid_list", context=invalid_context)
...
@mock.patch.dict(os.environ, {"OPENAI_API_KEY": "test_api_key"})
@mock.patch("app.agents.openai_research_wrapper.AGENTS_AVAILABLE", True) # Mock AGENTS_AVAILABLE in wrapper
@mock.patch("app.agents.openai_research_wrapper.run_research") # Patch run_research in the wrapper's module
async def test_execute_openai_research_success(mock_core_run_research_call):
    mock_core_report = ResearchReport(
        ...
        word_count=50,
        collected_facts=[{"fact": "core_fact1", "source": "core_src1", "timestamp": "ts1"}],
        error_message=None
    )
    mock_core_run_research_call.return_value = mock_core_report

    result_dict = await execute_openai_research(topic="wrapper_test_success", run_id="runSuccess123")
    ...
    mock_core_run_research_call.assert_called_once_with(topic="wrapper_test_success", trace_group_id="runSuccess123")
...
async def test_execute_openai_research_no_api_key_handled_by_core():
    ...
    with mock.patch("app.agents.openai_research_wrapper.run_research") as mock_core_run_research_no_key:
        mock_core_run_research_no_key.return_value = ResearchReport(
            ...
            outline=[], sources=[], word_count=0, collected_facts=[]
        )
        ...
        with mock.patch.dict(os.environ, {}, clear=True):
             result_dict = await execute_openai_research(topic="wrapper_no_key", run_id="runNoKey456")
...
async def test_execute_openai_research_core_logic_general_exception(mock_core_run_research_call):
    mock_core_run_research_call.side_effect = Exception("Core Logic Unexpected Boom!")

    result_dict = await execute_openai_research(topic="wrapper_core_general_exception", run_id="runGenExc789")

    assert result_dict["status"] == "error_uncaught_exception"
    assert "Core Logic Unexpected Boom!" in result_dict["error_message"]
    assert "Unhandled Exception" in result_dict["error_message"] # From wrapper's own except block

@mock.patch("app.agents.openai_research_wrapper.AGENTS_AVAILABLE", False) # Mock AGENTS_AVAILABLE in wrapper module
async def test_execute_openai_research_agents_sdk_not_available():
    ...
    result_dict = await execute_openai_research(topic="wrapper_no_sdk_avail", run_id="runNoSdk101")
